# C:\JWST-Mature-Galaxies\src\figures\plot_variance_figures_v4.py
# v4: Make z=10–20 histogram match the z=8–10 v5b style
#     - Legend INSIDE (upper-right, opaque)
#     - Dotted mock mean and μ+1σ
#     - Dashed red observed line
#     - Two-line title
#     - Inset zoom on mock peak (moved to lower-right, opaque)
#
# Run from src>:  python -m figures.plot_variance_figures_v4
...
# ---------- paths (project root = two levels up from figures/) ----------
HERE = os.path.dirname(__file__)
PROJ_ROOT = os.path.abspath(os.path.join(HERE, "..", ".."))  # -> C:\JWST-Mature-Galaxies
...
# ---------- z=10–20 histogram in v5b style ----------
def make_hist_z10_20_v4(run_dir: str):
    obs_csv, mv_csv = find_files(run_dir, "z10_20")
    ...
    # Same canvas size as z8–10 v5b
    fig, axs = fig_ax(1, 1, width_in=6.8, height_in=4.8)
    ax = axs[0] if isinstance(axs, (list, tuple, np.ndarray)) else axs
    ...
    ax.set_title("z = 10–20 (QC): normalized inter-field variance\nObserved vs. 5000 lognormal mocks")
    ticks_and_grid(ax)

    # Legend INSIDE (upper-right), fully opaque
    leg = ax.legend(frameon=True, loc="upper right", bbox_to_anchor=(0.86, 0.97))
    leg.get_frame().set_alpha(1.0)
    leg.get_frame().set_facecolor("white")

    # Inset zoom on the mock peak — moved to LOWER-RIGHT and fully opaque
    from mpl_toolkits.axes_grid1.inset_locator import inset_axes
    axins = inset_axes(
        ax,
        width="100%", height="100%",
        # (x0, y0, w, h) in axes coordinates; this avoids the red dashed line
        bbox_to_anchor=(0.55, 0.10, 0.40, 0.34),
    ...
    out = os.path.join(OUT_DIR, "variance_hist_z10_20_qc_v4")
