...
**Settings:** 10× oversampling, KDE redshift sampling, field stratification
...
### 3.4 Compute Correlation Functions ξ(d)
...
**Default settings** (Tukey α=0.25, mean detrending):
bash
python analysis/power_from_xi.py --stack

**Expected output:**
Loading ξ(d) from: ../results/step9c/run_YYYYMMDD_HHMMSS/
Processing tier: z4_6
...
This tests the stability of ξ(d) under photo-z uncertainties.
...
### 6.2 Compute ξ(d) for Resampled Catalogs
...
Null hypothesis testing for the z6p feature at λ ≈ 183 Mpc.

### 7.1 Find Your z6p ξ(d) File
...
Loading ξ(d) from: ../results/step9c/run_YYYYMMDD_HHMMSS/xi_z6p.csv
...
- [ ] `results/step9c/*/xi_z8_10.csv` shows positive ξ(d) at small scales
...
**Check:** Make sure you're using the timestamped folders from the SAME analysis run. Mixing DD from one run with RR from another will produce incorrect ξ(d).
